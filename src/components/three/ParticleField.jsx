...
export default function ParticleField({ count = 200, mousePosition = { x: 0, y: 0 } }) {
    ...
    const [particles] = useState(() => {
        ...
        return { positions, initialPos };
    });

    useFrame((state) => {
        if (!meshRef.current || !particles.initialPos) return;
        ...
        for (let i = 0; i < count; i++) {
            ...
            posArray[i * 3] = THREE.MathUtils.lerp(posArray[i * 3], particles.initialPos[i * 3] + offsetX * (i % 2 === 0 ? 1 : 0.5), 0.02);
            posArray[i * 3 + 1] = THREE.MathUtils.lerp(posArray[i * 3 + 1], particles.initialPos[i * 3 + 1] + floatY + offsetY * 0.5, 0.02);
